...
LATHA_FILE = join(OUTPUT_DIRECTORY, 'latha_file.txt')
...
if os.path.exists(OUTPUT_DIRECTORY):
	...
	if os.path.exists(LATHA_FILE):
		os.remove(LATHA_FILE)
else:
...
# print("ann files are ready")
count_multi_span = 0

relations_lines_latha=[]
relations_lines_labels=[]
...
for file in ann_data_files:
	...
	with open(NER_TRAINING_DATA_OUTPUT_PATH, 'a') as ner_training_data, open(RE_TRAINING_DATA_OUTPUT_PATH, 'a') as re_training_data, open(LATHA_FILE, 'a') as latha_file:
		for sentence in output['sentences']:
			...
			for sentence_row in sentence_re_rows:
				...
				token_count += 1
				sentence_latha.append(sentence_row['word'])

			re_training_data.write('\n')

			# writing relations to RE training data
			for relation in relations:
				new_line_latha = copy.deepcopy(sentence_latha)
				if relation['standoff_entity1_id'] in entities_in_sentence and relation['standoff_entity2_id'] in entities_in_sentence:
					...
					if(not sentence_latha[int(entity1)].startswith("@") and not sentence_latha[int(entity1)].startswith("$")):
						new_line_latha[int(entity1)] =  '@'+sentence_latha[int(entity1)]+'$'
					if(not sentence_latha[int(entity2)].startswith("@") and not sentence_latha[int(entity2)].startswith("$")):
						new_line_latha[int(entity2)] =  '@'+sentence_latha[int(entity2)]+'$'
					modif_sentence_latha = " ".join(new_line_latha)
					
					relations_lines_latha.append(modif_sentence_latha)
					relations_lines_labels.append(relation_name)
					# print(modif_sentence_latha, relation_name)
					latha_file.write('{}\t{}\n'.format(modif_sentence_latha, relation_name))
				new_line_latha = []
	...
	# print("Latha file ready")
	

import pandas as pd
final_df = pd.DataFrame({'sentence':relations_lines_latha, 'label':relations_lines_labels})
final_df.to_csv(join(OUTPUT_DIRECTORY, 'latha_file.csv'), index=False)
