"""
    Python script to run Supervised Fine-tuning on benchmark datasets: GAD, euadr, i2b2 2010
...
os.environ["WANDB_PROJECT"]="sft-benchi-llama"
# os.environ["WANDB_MODE"]="dryrun"

# Configure the logging settings
logging.basicConfig(level=logging.INFO, filename='sft-benchi-llama.log', filemode='a',
...
def train_model(model, data, benchi,optimizer=AdamW, b_size=batch_size, epochs=num_epochs):
    """ Train the model on the given dataset 
    Params:
        model: model to train
        data: (mode=train) tuple of train and validation dataset; (mode=test) test dataset
        optimizer: optimizer to use = AdamW (default)
        batch_size: batch size = 2 (default) 
        epochs: number of epochs = 2 (default)   
    """
    global step
    best_val_f1 = None
    impatient=0
    ...
    model.train()
    optim = optimizer(model.parameters(), lr=lr, eps=1e-8)
    ...
    model.save_pretrained(model_path+ benchi +'/')

    print("Saving loss dict")

    with open(benchi+'_loss_dict.pickle', 'wb') as handle:
        loss_dict["train_loss"] = train_loss_list
        loss_dict["val_loss"] = val_loss_list
        loss_dict["val_f1"] = val_f1_list
